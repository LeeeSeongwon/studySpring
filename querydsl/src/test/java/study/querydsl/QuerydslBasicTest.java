...
class QuerydslBasicTest {
    ...
    // // member1을 찾아라.
    // Member findMember = (Member) em.createQuery("select m from Member m where
    // m.username = :username")
    ...
    // /**
    // * 예) 회원과 팀을 조인, 팀 이름이 teamA만 조인, 회원은 모두 조회
    ...
    // List<MemberDto> result = queryFactory
    // .select(Projections.fields(MemberDto.class, // getter setter 없어도 됨
    // member.username,
    // member.age))
    // .from(member)
    // .fetch();
    ...
    // member.username.as("name"), // alias 맞춰야함
    // member.age))
    // .from(member)
    // .fetch();
    ...
    // // 영속성 context로 인해 변경값이 적용안됨 em.flush, clear 해줘야함
